...
with st.expander("◆ 왜 ‘다층 방어(Defense-in-Depth)’인가?"):
    st.markdown(
        "- **L1 · 룰 레이어**: 빠르고 무료·오프라인. *알려진* 공격·base64는 즉시 차단하지만 "
        "**패러프레이즈·다국어·글자조작·base64 외 인코딩이면 그냥 통과**합니다(시그니처 룩업이라 일반화 안 됨).\n"
        ...
        "**⚠️ 정직 고백:** 이 배포 데모는 키가 없어 **L1(룰) 단독**으로 돕니다 — 위 약점에 그대로 노출됩니다. "
        "그래서 진짜 메시지는 ‘룰로 다 막힌다’가 아니라 ‘룰만으론 못 막는다 → L2·다층이 필요하다’입니다. "
        "위험도 점수도 경험적 보정 없는 휴리스틱 가중치입니다.\n\n"
