...
with st.expander("◆ 왜 ‘다층 방어(Defense-in-Depth)’인가?"):
    st.markdown(
        "- **L1 · 룰 레이어**: 빠르고 무료·오프라인. 시그니처 + **매칭 전 역난독화**(NFKC·제로폭·동형문자·공백복원·리트스피크·base64/hex/ROT13)로 "
        "리트스피크·글자조작·인코딩 우회까지 차단하지만, **다국어·의미 패러프레이즈는 통과**합니다(번역·의미는 룰 밖).\n"
        ...
        "**⚠️ 정직 고백:** 이 배포 데모는 키가 없어 **L1(룰) 단독**으로 돕니다. 난독화 우회는 막지만 "
        "**다국어·패러프레이즈는 L2 없이 못 막습니다**(같은 공격도 일본어면 통과). 위험도 점수도 경험적 보정 없는 휴리스틱입니다. "
        "결론: ‘룰로 다 막힌다’가 아니라 ‘룰을 강화해도 의미 우회는 남는다 → 다층이 필요하다’.\n\n"
